...
usage() {
  this=$1
  cat <<EOF
$this: download pre-compiled Docker images for rosetta-dogecoin/rosetta-dogecoin
...
execute() {
  ...
  docker tag "rosetta-dogecoin:${TAG}" "rosetta-dogecoin:latest"
  log_info "loaded rosetta-dogecoin:${TAG} and tagged as rosetta-dogecoin:latest"
  rm -rf "${tmpdir}"
...
BINARY=rosetta-dogecoin
FORMAT=tar.gz
OWNER=rosetta-dogecoin
REPO=rosetta-dogecoin
PREFIX="$OWNER/$REPO"
